...
from test.integration.test_methods import (
    create_storage,
    create_sensor,
    get_database_connection,
)
from test.rest_functionality.rest_methods import post_request, delete_request
...
def test_storage():
    ...
    with conn.cursor() as cursor:
        cursor.execute(
            'SELECT "name" FROM "Storage" WHERE id = %s', (storage_id,)
        )
...
def test_sensors():
    ...
    with conn.cursor() as cursor:
        cursor.execute(
            'SELECT "name" FROM "Sensor" WHERE id = %s', (sensor_id,)
        )
        assert cursor.fetchone() is None
    delete_request(f"storages/{storage_id}")
...
def test_measurements():
    ...
    assert any(
        ...
        for measurement in get_request(f"measurements/{sensor_id}")
    )
    assert len(old_measurements) + 1 == len(
        get_request(f"measurements/{sensor_id}")
    )
    delete_request(f"sensors/{sensor_id}")
    delete_request(f"storages/{storage_id}")
